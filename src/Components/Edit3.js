...
function EditPost() {
    ...
    const getPostById = async() => {
        ...
        const response = await axios.get(`http://localhost:8000/api/page4/${id}`);
        //get response data
    ...
    const updatePost = async (e) => {
        ...
        //send data to server
        await axios.put(`http://localhost:8000/api/page4/${id}`, {
            title: title,
            desc: desc
        })
        .then(() => {
            ...
            <Row>
                <Col md="{12}">
                    <Card className="border-0 rounded shadow-sm">
                        <Card.Body>
                        <Button as={Link} to="/dhasboard3" variant="danger" className="mb-3">Back</Button>
                            {
                                validation.errors &&
